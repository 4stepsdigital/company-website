...
const overviewApi = async (req, res) => {
    ...
    if (req.method === "POST") {
        try {
            const data = await ProductHighLights.findOneAndReplace(
                { product },
            ...
            await Product.findByIdAndUpdate({_id:product}, { $set: { highlight: data?._id } });
        ...
        try {
            ...
            const data = await ProductHighLights.findOne({ product });
