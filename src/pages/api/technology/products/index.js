...
// const uploadDirectory = "./public/uploads/technology/technologyProducts";
// if (!fs.existsSync(uploadDirectory)) {
//   fs.mkdirSync(uploadDirectory, { recursive: true });
// }

const uploadDirectory = path.join(process.cwd(), "uploads/technology/technologyProducts"); // Define your upload directory
  ...
  if (req.method === "POST") {
    upload.single("file")(req, res, async (err) => {
      ...
      const fileData = {
        ...
        path: req.file ? `/api/uploads/technology/technologyProducts/${req.file.filename}` : null,
      };
      

      try {
        // Save data to the database
        const newFile = await SubTechnologyProduct.create(fileData);
    ...
    try {
      const file = await SubTechnologyProduct.findById(id);
      if (!file) {
      ...
      await SubTechnologyProduct.findByIdAndDelete(id);
    ...
    try {
      const files = await SubTechnologyProduct.find({ technology: id });
