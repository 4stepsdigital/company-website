  ...
  if (req.method === "POST") {
    upload.single("file")(req, res, async (err) => {
      ...
      const fileData = {
        ...
        path: req.file ? `/api/uploads/technology/technologyServices/${req.file.filename}` : null,
      };
      

      try {
        // Save data to the database
        const newFile = await SubTechnologyServices.create(fileData);
    ...
    try {
      ...
      await SubTechnologyServices.findByIdAndDelete(id);
    ...
    try {
      const files = await SubTechnologyServices.find({ technology: id });
