...
const uploadDirectory = path.join(process.cwd(), "uploads/industry/industrysuccess"); // Define your upload directory
...
const apiRoute = async (req, res) => {
  ...
  if (req.method === "PUT") {
    upload.single("file")(req, res, async (err) => {
      ...
      try {
        const existingData = await Success.findById(id);
        if (!existingData) {
        ...
        if (req.file) {
          ...
          fileData = {
            ...
            path: `/api/uploads/industry/industrysuccess/${req.file.filename}`,
          };
        }
        ...
        const updatedFile = await Success.findByIdAndUpdate(id, updatedData, { new: true });
