...
const overviewApi = async (req, res) => {
    ...
    if (req.method === "POST") {
        try {
            const data = await ResultMatrix.findOneAndReplace(
                { casestudy },
            ...
            await CaseStudy.findByIdAndUpdate({_id:casestudy}, { $set: { result: data?._id } });
        ...
        try {
            ...
            const data = await ResultMatrix.findOne({ casestudy });
