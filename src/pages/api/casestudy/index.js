...
const apiRoute = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    upload.single('file')(req, res, async (err) => {
      ...
      if (fileData) {
        try {
          const existingFile = await CaseStudyPromo.findOne();

          if (existingFile) {
            // Update existing file
            const updatedFile = await CaseStudyPromo.findOneAndUpdate(
            ...
            const file = new CaseStudyPromo(fileData);
            await file.save();
    ...
    try {
      ...
      const files = await CaseStudyPromo.findOne().populate("seo");
