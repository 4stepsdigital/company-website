...
const apiRoute = async (req, res) => {
  ...
  if (req.method === "POST") {
    upload.single("file")(req, res, async (err) => {
      ...
      const {
        title,editorHtmlDescription: editorHtmlDescriptionRaw, service
      } = req.body;
    ...
    //   console.log("solution solution Item ------> ",solution)
      ...
      const fileData = {
        ...
        path: req.file ? `/api/uploads/service/serviceOvervies/${req.file.filename}` : null,
      };

      try {
        const updatedFile = await ServiceOverviewItem.create(fileData);
    ...
    try {
      ...
      await ServiceOverviewItem.findByIdAndDelete(id);
    ...
    const { id } = req.query;
    try {
      const files = await ServiceOverviewItem.find({service:id});
