...
const apiRoute = async (req, res) => {
  ...
  if (req.method === "PUT") {
    ...
    upload.single("file")(req, res, async (err) => {
      ...
      const { title, link, editorHtmlDescription: editorHtmlDescriptionRaw, service } = req.body;
      ...
      const fileData = {
        ...
        path: req.file ? `/api/uploads/service/serviceSolution/${req.file.filename}` : file?.path,
      };
        ...
        const newFile = await SubServiceSolution.findOneAndUpdate({_id:id},{$set:fileData});
