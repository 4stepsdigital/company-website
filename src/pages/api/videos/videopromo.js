...
// const uploadDirectory = './public/uploads/videopromo';
// if (!fs.existsSync(uploadDirectory)) {
//   fs.mkdirSync(uploadDirectory, { recursive: true });
// }

const uploadDirectory = path.join(process.cwd(), "uploads/VideoPromo"); // Define your upload directory
  ...
  if (req.method === 'POST') {
    ...
    upload.single('file')(req, res, async (err) => {
      ...
      const fileData = req.file ? {
        ...
        videoPath: `/api/uploads/videopromo/${req.file.filename}`,
      } : null;

      if (fileData) {
        try {
          const existingFile = await VideoPromo.findOne();

          if (existingFile) {
            // Update existing file
            const updatedFile = await VideoPromo.findOneAndUpdate(
            ...
            const file = new VideoPromo(fileData);
            await file.save();
    ...
    try {
      ...
      const files = await VideoPromo.findOne().populate("seo");
