...
const apiRoute = async (req, res) => {
  ...
  if (req.method === "POST") {
    upload.single("file")(req, res, async (err) => {
      ...
      const {
        title,editorHtmlDescription: editorHtmlDescriptionRaw, solution
      } = req.body;
    ...
    //   console.log("solution solution Item ------> ",solution)
      ...
      const fileData = {
        ...
        path: req.file ? `/api/uploads/solution/SolutionOverview/${req.file.filename}` : null,
      };
    ...
    const { id } = req.query;
    try {
      const files = await SolutionOverviewItem.find({solution:id});
